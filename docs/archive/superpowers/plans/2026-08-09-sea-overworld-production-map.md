...
**Goal:** 以 `sea_overworld_stage1_graybox_v4.png` 为构图唯一依据，制作并接入四分块生产大地图，使 15 个地点、碰撞、入口、海图和现有航行交互全部与新画面一致。
...
- 地点分布固定为 A4、B5、C4、D2；B 是右上群岛水战区，D 是右下敌方核心海域。东极秘岛因没有独立生产岛体，经用户批准从运行时删除。
...
- [x] 将 `docs/design/sea-overworld-design.md` 的当前阶段更新为“v4 构图生产落地”，明确 B/D 正确职责、四分块尺寸、接缝和当前 15 地点。
...
- [x] 将 15 个地点更新为已批准坐标：
  ...
  | 区域 | 地点与世界坐标 |
  |---|---|
  | A | 南海军港 `(1080,650)`；川山渔村 `(480,1040)`；东湾水寨 `(2040,520)`；青屿秘境 `(2380,540)` |
  ...
  | D | 红湾卫所 `(2980,1760)`；南澳商港 `(4380,2460)` |
...
- [x] 以生产底图的世界坐标重新描绘西北大陆海岸 `CollisionPolygon2D`，保留海湾入口与沿岸航道。
- [x] 为 15 个可进入地点及其所在主要陆地建立贴合轮廓的复合圆形或多边形阻挡，不用一个过大的圆覆盖港池、码头入口或岛间海峡。
- [x] 为不可进入山岛、礁群和中央微型礁石只添加必要阻挡；背景中的沉船保持视觉元素，不新增玩法或伤害。
- [x] 检查四条必通路线：A→B、A→C、B→D、C→D；中央堡垒上下、左右都必须可绕行，连续可航水门宽度不少于约 `480` 世界单位。
- [x] 将两艘事件船和漂流事件移到明确水面，避免与新岛屿碰撞、地点触发或场景二出生点重叠。
- [x] 使用玩家真实 `CharacterBody2D` 沿北线、南线、四条接缝和中央堡垒四向走廊完成 8 条航路 smoke test，确认没有隐形墙、穿岛、死胡同或摄像机越界。

## Task 5：更新针对性测试和视觉验收

- [x] 修改 `tests/test_sea_overworld.gd` 的资源断言，要求四张 v3 分块存在、尺寸正确，并继续验证 A/B/C/D 背景节点序列化完整。
- [x] 将旧 `_verify_east_expansion()` 与 `_verify_d_expansion()` 的职责改为新布局：B 断言 5 个右上地点且 `y < 1292`，D 断言 2 个右下地点且 `y > 1292`；C 仍为 4 个，A 仍为 4 个。
- [x] 增加 15 个地点名称、批准坐标容差 `±80`、月环商港左侧入口、南海军港出生点不碰撞的断言。
...
- [x] 更新 `docs/qa/playtest.md`，加入北线、南线、中央通航、15 个地点交互和完整海图检查项。
...
- 15 个地点标签、入口和占位提示全部落在对应岛屿，不存在 B/D 旧布局残留。
