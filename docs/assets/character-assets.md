...
场景一与场景二共用确定的角色帧数量清单：主角四方向待机各 16 帧、行走依次为下 11 / 左 20 / 右 20 / 上 8；皇帝向下待机 16 帧，其余待机和行走各 4 帧；士兵和县令四方向待机各 2 帧、行走各 9 帧。帧号均从 `1.png` 连续编号。运行时通过 Godot 资源加载器读取这些路径及人物立绘，确保导出后的 `.ctex` 纹理映射仍可用。
...
- 256/512 像素源帧只作为仓库外的美术交付源图；GitHub 仅保留接入运行时的标准帧。接入时按透明内容等比缩放到现有约 50 像素可见高度并统一脚底，不拉伸、不改变动作顺序。
- 早期 `protagonist_candidate` 候选目录已完成筛选作用，正式帧已进入 `protagonist/standard`；候选图、预览、原始表和生成元数据于 2026-09-07 从工作树删除，可从 Git 历史恢复。
