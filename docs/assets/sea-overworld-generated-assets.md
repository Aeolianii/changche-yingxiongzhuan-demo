...
当前 A/B/C/D v3 生产图将海水、岩岸、岛屿和建筑烘焙在同一张母图中。由于生成图内部建筑线条含有低清伪细节，单纯提高纹理导入质量或放大现有图片不能恢复清晰结构。

后续高清化采用独立分层生产：四张可拼接纯海洋背景只承载海色、轻微波纹和无方向性的低对比纹理；大陆海岸、15 个活动地点及必要装饰岛礁分别生成独立高清主体，以透明 PNG 在 Godot 中摆放。所有地点继续服从 `sea_overworld-stage1-layout.md` 的坐标、功能轮廓与入口方向，且不得在图片中烘焙地点文字、航线、角色、船只、UI 或碰撞信息。

首个校准样张为“月环商港”：保留当前独立岛的低宽月牙轮廓、朝左港池入口和高密度商港建筑，用它优先验证放大清晰度与透明边缘。在样张通过前不批量生成其余地点，以避免视角、建筑比例、光照和边缘风格整体漂移。新资产不得覆盖 A/B/C/D v3，统一存放于 `assets/sprites/sea_overworld/hd_locations/`。
