...
2026-09-07 进一步执行“仅保留运行必需素材”：阶段一构图灰模和独立地点高清参考图未接入任何运行时场景，已从工作树删除；历史构图说明移至 `docs/archive/assets/sea-overworld-stage1-layout.md`，图片仍可从 Git 历史恢复。

### 1.1 v4 生产运行时分块

四张分块最初以 v4 灰模为构图参考，通过内置生图工具的 `sketch-to-render` 模式生成统一生产画面，再由 `tools/slice_sea_overworld_map.py` 按 `160` 源像素重叠带确定性裁切。GitHub 只保留当前运行时加载的 A/B/C/D v3 分块；灰模、母图和生成中间稿均不随交付仓库上传。
...
### 高清分层参考方案（2026-08-10，未接入）

该方案曾尝试把大陆海岸、活动地点和装饰岛礁拆成独立透明 PNG，但所有样张均未接入代码、场景、数据或测试。为保持 GitHub 交付只包含运行必需素材，`assets/sprites/sea_overworld/hd_locations/` 已于 2026-09-07 删除；当前运行时继续使用 A/B/C/D v3 生产分块。

本节后续提示词仅用于追溯生成过程，不代表仓库应包含对应参考图片。需要恢复时应从 Git 历史提取，不应重新加入正式运行素材目录。
