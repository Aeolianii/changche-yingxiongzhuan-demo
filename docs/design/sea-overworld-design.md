...
3. 玩家点击“进入”按钮或按E后，伏波古岭切换到真实岛屿场景；其他地点仍提示“该地点即将开放”。
4. 玩家接触海上事件或船只后自动触发提示，显示“该事件开发中”或“该船只开发中”。

当前初版只为伏波古岭接入真实岛屿小地图与码头返回闭环；其他地点不切换小地图、不播放剧情、不进入战斗。完整海图只负责查看地点与玩家位置，不实现快速移动、区域解锁和潮汐通道逻辑。这些内容只作为后续方向保留在本文档中。
...
当前运行时使用同一张生产母图裁出的 A/B/C/D v3 四分块，并通过 `120` 世界单位重叠带连接成二乘二海域。地图共有 15 个可进入地点、两艘自动触发船只和一个漂流事件；进入交互继续复用现有主动点击或按 E 的占位流程。

### 1.3 v4 生产落图状态

右上 B 区为群岛水战区，配置沧门礁堡、月环商港、雾岚群岛、伏波古岭与珊湾渔链五个地点；右下 D 区为敌方核心海域，只配置红湾卫所与南澳商港两个地点。A 区和 C 区各四个地点，总数为 15；东极秘岛因生产底图中没有独立岛体而删除，四张背景、批准坐标、入口触发、场景二出生点和完整海图资源已完成接入。
...
- D 区承担敌方核心海域，红湾卫所与南澳商港形成“前置要塞—终局港口”的两层结构，右下大型港城成为终局视觉地标。
- 当前 15 个地点名称和交互职责保持稳定；东极秘岛不再占用南澳商港城塞的同一岛体。

北线为“南海军港—东湾水寨—沧门礁堡—雾岚群岛—月环商港”，再向南进入 D 区；南线为“川山渔村—澄海灯岛—龙门海寨—玄潮古屿—红湾卫所”。两线最终在 D 区于红湾卫所汇合并抵达南澳商港。四条分块接缝都必须用岛链、礁石走向或航道开口建立跨区联系，避免出现独立画布感。
...
- 新资源使用 `guangdong_sea_zone_[a-d]_v3.png` 版本化命名，不覆盖旧生产背景或 v1～v4 灰模。
...
- 同一次接触只触发一次，避免提示在每一帧重复弹出；可重复事件必须驶离后再次进入才能重新触发。
- 漂流木箱是一次性事件：玩家接触后停止航行，选择“打捞上来”或“置之不理”；打捞显示金石 `+100`、木材 `+100`、银钱 `+1000` 的原型奖励提示，任一分支结束后木箱永久移除。
- 龙井茶商与私盐商使用纸质双选项对话，按历史原型规则提供交易或拒绝分支；当前银钱变化只属于海图运行时与场景存档快照，不接入完整经济系统。
- 尚未制作的普通事件继续提示“【事件名称】开发中”。
- 当前船只统一提示“【船只名称】开发中”。
...
海图采用永久探索迷雾。新游戏默认揭示左上大陆轮廓和南海军港出生点的当前相机视野；大陆外未探索海域显示为黑色。玩家航行时以当前相机可见世界矩形持续揭示，已经揭示的区域不会重新变黑。航行场景与完整海图复用同一探索数据，完整海图只显示已揭示地点。

航行场景使用原始探索位图保证相机视野完整点亮；完整海图可以对同一位图应用仅影响显示的圆润、水墨扰动柔边，但不得改变探索判定或提前显示未探索地点。
...
探索进度跨场景二和伏波古岭往返保留，并写入正式单槽存档。旧存档缺少迷雾数据时，按默认大陆和出生视野初始化；开始新游戏清空运行时探索数据但不删除磁盘旧存档。
