...
- 海盗船接触主角时进入战斗入口。先锁定海图并弹出 4 个难度选项：难度一 / 难度二 / 难度三 / 公式难度（公式难度按玩家经济舰队强度计算，`强度分 = Σ每艘(1 + 平均 upgrades 等级)`，`≤6→1`、`≤12→2`、否则 `3`；读取不到舰队时默认难度 2）。选择后生成对应难度的随机遭遇战并进入正式海战；战斗结算后返回海上大地图：胜利 → 移除本次接触的海盗船；失败/逃跑/平局 → 保留海盗，玩家船在月环商港附近复活并提示休整。
...
航行场景与完整海图直接复用海战透明白色像素水墨海雾组件 `white_ink_mist_v1.png`。8 世界单位细格继续记录永久揭示；两处雾笔表现都严格使用海战 `CellSize = 26`，航行画面解释为 26 世界单位，完整海图解释为 26 海图像素。两处均按实时隐藏状态计算九宫格邻居，并共同使用由 `Hash01` 经确定性雪崩混合得到的雾种子、4.4–5.5 格宽、0.6875 高宽比、固定位置偏移、0.38 Alpha 和边缘 0.76 衰减。落点改为每个 2×2 表现格由独立混合哈希选择一个候选，消除原 `seed % 4` 低两位造成的严格斜线周期，同时固定维持四分之一密度，避免纯随机产生大空洞。为避免素材自身的固定斜向笔触在大面积叠加后形成平行线，同一尺寸额外预生成原图、水平镜像、垂直镜像和双向镜像四种方向，并由雾种子稳定选择；这些处理只打散纹理分布，不改变战斗雾的尺度。探索状态变化后同步重建两张雾笔场，雾笔中心进入已探索区时整张移除。航行场景使用原始探索位图；完整海图在同一探索位图上追加仅用于显示的局部 Alpha 平滑。视觉柔边和雾笔纹理不能改变实际探索状态，也不能让未探索地点名称提前出现。

完整海图的地图分块、探索遮罩、地点标签和玩家标记均非等比映射到整个 870×510 内框；允许相对原地图比例做轻微纵向拉伸，以消除内框上下留白。各层必须使用相同 X/Y 映射，不能产生地点或迷雾错位。

未知区域不能直接以半透明雾叠在真实地图上。探索遮罩覆盖处先绘制世界坐标固定的高分辨率水墨海水遮蔽底层，再在其上混合分格白雾；遮蔽海面沿用大地图原有青蓝海色，并用生成素材的亮度调制细小亮纹、暗流和短碎波线，按完整世界 UV 单次铺设。雾笔孔洞只显露该海水底层，不能提前显露岛屿、港口或大陆轮廓。探索遮罩清除后，海水遮蔽和白雾一起移除，真实地图内容才显示。

完整海图的柔边允许叠加稳定的低频水墨扰动，使轮廓产生小幅凹凸和错落，避免连续直边仍呈现矩形拼块感。粗格雾笔允许在未知海域内部留下少量不规则小孔洞和明显浓淡层次，但禁止出现可辨认的矩形平铺接缝、固定斜向平行纹、规则蓝色间隔或大面积连续透海空白；扰动和方向变体不能随时间变化，也不能改变探索状态。
