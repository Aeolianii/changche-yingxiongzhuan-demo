...
航行场景与完整海图直接复用海战透明白色像素水墨海雾组件 `white_ink_mist_v1.png`，但探索逻辑与迷雾表现分格。8 世界单位细格继续记录永久揭示；迷雾美术使用约 84、56、40 世界单位三套表现格，分别预生成背景、中景、细节世界雾场。三层按海战同款确定性哈希布置雾纹，但使用不同哈希盐、透明度、尺寸和镜像方向，使海图由大量中小尺度雾团组成而不重复。探索位图只负责裁切三层雾场，不再以单张超大云块或高底雾填满画面。航行场景使用原始探索位图，确保当前相机视野完整点亮；完整海图可以在同一位图上追加仅用于显示的局部 Alpha 平滑，使探索边缘和直角转折略微圆润。视觉柔边和雾笔纹理不能改变实际探索状态，也不能让未探索地点名称提前出现。
