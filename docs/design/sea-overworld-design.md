...
当前运行时使用同一张生产母图裁出的 A/B/C/D v3 四分块，并通过 `120` 世界单位重叠带连接成二乘二海域。地图共有 15 个可进入地点、一个固定普通商船目标，以及由茶叶商、私盐商、漂流木箱和雾中海怪组成的两个动态随机事件槽位；进入交互继续复用现有主动点击或按 E 的占位流程。
...
- 海图随机事件同时最多出现两个。每次进入海图都重新抽取事件和刷新位置，不沿用上一次海图实例、伏波返航上下文或场景快照中的活动事件；任一事件完整结算后，在玩家当前相机视野外补充一个事件，使活动槽位恢复为两个。
- 茶叶商事件的完成状态单独写入全局世界状态并随正式存档保留。尚未完成时，茶叶商固定占用一个事件槽；完成后不再强制刷新。其余槽位从私盐商、漂流木箱和“雾中可疑身影”中抽取，同一时刻每种事件最多一个实例。
- “雾中可疑身影”在海图上从三张透明背景的浓白雾、低对比淡黑剪影中随机选择一种，并用边缘透明衰减自然融入海面。亮色雾层通过时间驱动的低速噪声产生缓慢漂移和轻微明暗呼吸，暗色海怪剪影保持稳定，避免轮廓扭曲或闪烁；接触前不显示海怪颜色、面部或材质细节。玩家接触后可选择“靠近查看”或“绕行”；靠近才揭示对应海怪立绘，并提供“战斗系统尚未接入，此战默认获胜”的占位战斗选项。胜利后显示战胜海怪，真实获得木材 `500`、铁石 `500`；绕行无奖励。
