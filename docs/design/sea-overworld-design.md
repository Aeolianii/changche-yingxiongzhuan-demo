...
航行场景与完整海图都复用海战现有的透明白色像素水墨海雾组件 `white_ink_mist_v1.png`，探索位图只负责裁切雾层 Alpha。航行场景使用原始探索位图，确保当前相机视野完整点亮；完整海图可以在同一位图上追加仅用于显示的局部 Alpha 平滑，使探索边缘和直角转折略微圆润。视觉柔边和白雾纹理不能改变实际探索状态，也不能让未探索地点名称提前出现。

完整海图的柔边允许叠加稳定的低频水墨扰动，使轮廓产生小幅凹凸和错落，避免连续直边仍呈现矩形拼块感。白雾素材以错位、镜像的多层采样拼合，禁止出现可辨认的矩形平铺接缝；扰动不能随时间变化，也不能在已探索区域内部生成雾孔或噪点。
