...
当前初版不切换岛屿小地图、不播放剧情、不进入战斗；完整海图只负责查看地点与玩家位置，不实现快速移动、区域解锁和潮汐通道逻辑。这些内容只作为后续方向保留在本文档中。
...
当前运行时使用同一张生产母图裁出的 A/B/C/D v3 四分块，并通过 `120` 世界单位重叠带连接成二乘二海域。地图共有 16 个可进入地点、两艘自动触发船只和一个漂流事件；进入交互继续复用现有主动点击或按 E 的占位流程。

### 1.3 v4 生产落图状态

右上 B 区为群岛水战区，配置沧门礁堡、月环商港、雾岚群岛、伏波古岭与珊湾渔链五个地点；右下 D 区为敌方核心海域，配置红湾卫所、东极秘岛与南澳商港三个地点。A 区和 C 区各四个地点，总数保持 16；四张背景、批准坐标、入口触发、场景二出生点和完整海图资源已完成接入。
