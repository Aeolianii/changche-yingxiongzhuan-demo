...
航行场景与完整海图直接复用海战透明白色像素水墨海雾组件 `white_ink_mist_v1.png`。8 世界单位细格继续记录永久揭示；两处雾笔表现都严格使用海战 `CellSize = 26`，航行画面解释为 26 世界单位，完整海图解释为 26 海图像素。两处均按实时隐藏状态计算九宫格邻居，并完整复用 `Hash01(x,y,53)`、每约四格一张、4.4–5.5 格宽、0.6875 高宽比、固定位置偏移、0.38 Alpha 和边缘 0.76 衰减。探索状态变化后同步重建两张雾笔场，雾笔中心进入已探索区时整张移除。航行场景使用原始探索位图；完整海图在同一探索位图上追加仅用于显示的局部 Alpha 平滑。视觉柔边和雾笔纹理不能改变实际探索状态，也不能让未探索地点名称提前出现。
