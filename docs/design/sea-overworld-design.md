...
- 风向系统。
- 影响航行速度、碰撞或路线判断的天气浓雾系统。海图探索使用的白色水墨战争迷雾不属于天气系统。
- 浅滩与吃水深度系统。
- 暗礁碰撞与船体损伤系统。
- 洋流加减速系统。
- 风暴区和持续航行消耗。
...
- 船上主角四方向形象使用 8.22 新主角动作的对应朝向首帧制作，保持既有四格图集画布和 `0.088` 显示比例；站位按船体各方向的实际可见中线校正，其中向下、向上分别补偿 `+3px`、`-9px` 横向偏移，确保人物位于甲板中央。只调整人物锚点，不重绘船体与尾流。
...
- 龙井茶商与私盐商使用各自 8.22 新立绘的纸质选项对话，分别提供交易/拒绝与查扣/受贿/放行分支；结果中的军饷、龙井茶和私盐已接入全局经济仓库。任一分支结束后对应商船移除，单次经济结算仍通过 `GameState` 完成。
- 海上遇敌、漂流木箱、商船、海怪和倭寇营地最终战共用的事件对话，应让长中文正文与详情在人物立绘之间的固定安全行宽内换行；Windows 发布版与 Godot 工程显示一致，不裁掉正文、选项或底板。
...
- 海盗船接触主角时进入战斗入口。先锁定海图，由水师士兵报告“前方遇敌”，玩家继续后切换到 8.22 海盗小兵立绘，由其叫阵“兄弟们，抄家伙，干他们一票！”；随后保持原 4 个难度选项：难度一 / 难度二 / 难度三 / 公式难度（公式难度按玩家经济舰队强度计算，`强度分 = Σ每艘(1 + 平均 upgrades 等级)`，`≤6→1`、`≤12→2`、否则 `3`；读取不到舰队时默认难度 2）。选择后生成对应难度的随机遭遇战并进入正式海战；战斗结算后返回海上大地图：胜利 → 移除本次接触的海盗船；失败/逃跑/平局 → 保留海盗，玩家船在月环商港附近复活并提示休整。
- 海盗分布不写入存档，返回海上大地图时重新随机生成 5 艘；胜利后对应同名海盗不再生成（海盗数减一），失败后按同名海盗恢复巡航。
- 海盗战和海怪讨伐战返回海图时，先从全局世界状态恢复主线阶段及倭寇营地警告确认状态，再处理战斗结算；已经确认过的营地警告不会因为战斗切场而重新弹出。
...
A/B/C/D 地图分块使用 X/Y 独立缩放铺满整个 870×510 中央内框，允许相对世界地图比例做轻微纵向拉伸，不在内框上下保留空位。地点标签、玩家标记与迷雾层必须使用同一非等比映射，保证拉伸后仍与地图内容对齐。

右上返回按钮使用透明生成式像素水墨横笔触作为底板，由 Godot 在笔触安全区内居中绘制“返回”二字；不再使用规则长方形金边框。按钮点击范围覆盖完整笔触，同时保留 Esc 与 E 键关闭海图。

海图采用永久探索迷雾。新游戏默认揭示左上大陆完整轮廓，以及南海军港出生点当前相机圆角矩形视野；大陆外的未探索海域由半透明白色像素水墨海雾覆盖。玩家驾驶船只经过时，以当前相机圆角矩形视野持续揭示海图：四边保持主要可见范围，四个尖角按圆弧裁切，已经揭示的区域不会重新被雾覆盖。航行场景和完整海图必须复用同一份探索数据，不能出现一边已知、一边仍被雾覆盖的状态。

航行场景与完整海图直接复用海战透明白色像素水墨海雾组件 `white_ink_mist_v1.png`，但探索逻辑与迷雾表现分格。8 世界单位细格继续记录永久揭示；约 224 世界单位粗格只负责雾笔美术，按海战同款确定性哈希每约四格布置一张 4.4–5.5 格宽、带稳定尺寸和位置偏移的雾纹，预生成世界坐标固定的密度纹理。探索位图只负责裁切该纹理 Alpha，不再以六层全屏贴图和高底雾填满孔洞。航行场景使用原始探索位图，确保当前相机视野完整点亮；完整海图可以在同一位图上追加仅用于显示的局部 Alpha 平滑，使探索边缘和直角转折略微圆润。视觉柔边和雾笔纹理不能改变实际探索状态，也不能让未探索地点名称提前出现。

未知区域不能直接以半透明雾叠在真实地图上。探索遮罩覆盖处先绘制世界坐标固定的水墨海水遮蔽底层，再在其上混合分格白雾；雾笔孔洞只显露该海水底层，不能提前显露岛屿、港口或大陆轮廓。探索遮罩清除后，海水遮蔽和白雾一起移除，真实地图内容才显示。

完整海图的柔边允许叠加稳定的低频水墨扰动，使轮廓产生小幅凹凸和错落，避免连续直边仍呈现矩形拼块感。粗格雾笔允许在未知海域内部留下少量不规则小孔洞和明显浓淡层次，但禁止出现可辨认的矩形平铺接缝、规则蓝色间隔或大面积连续透海空白；扰动不能随时间变化，也不能改变探索状态。
