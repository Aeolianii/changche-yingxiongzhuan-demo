...
海图采用永久探索迷雾。新游戏默认揭示左上大陆完整轮廓，以及南海军港出生点当前相机可见世界矩形；大陆外的未探索海域由半透明白色像素水墨海雾覆盖。玩家驾驶船只经过时，以当前相机可见世界矩形持续揭示海图，包含屏幕四角和边缘，已经揭示的区域不会重新被雾覆盖。航行场景和完整海图必须复用同一份探索数据，不能出现一边已知、一边仍被雾覆盖的状态。
