...
当前运行时使用同一张生产母图裁出的 A/B/C/D v3 四分块，并通过 `120` 世界单位重叠带连接成二乘二海域。地图共有 15 个可进入地点、一个固定普通商船目标，以及由茶叶商、私盐商和漂流木箱组成的两个动态随机事件槽位；进入交互继续复用现有主动点击或按 E 的占位流程。
...
- 海图随机事件同时最多出现两个。新海图实例固定生成茶叶商船，并在私盐商船与漂流木箱中随机生成一个；任一事件完整结算后，在玩家当前相机视野外从未激活类型中补充一个事件，使活动槽位恢复为两个。
- 同一事件类型在被移除后可以参与后续轮换，但同一时刻每种事件最多一个实例。当前活动事件的类型与位置写入场景快照及伏波返航上下文，避免读取或返航时重复生成。
- 私盐商船围绕每次刷新点在半径 `120` 世界单位内平滑巡航；进入事件对话后暂停，避免交互过程中继续漂离玩家。
