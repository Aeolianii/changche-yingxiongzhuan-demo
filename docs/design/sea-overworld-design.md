...
海图采用永久探索迷雾。新游戏默认揭示左上大陆完整轮廓，以及南海军港出生点当前相机圆角矩形视野；航行大地图的未探索海域继续使用半透明白色像素水墨海雾，点击右下角按钮进入完整海图后，未探索区域改为纯黑覆盖。玩家驾驶船只经过时，以当前相机圆角矩形视野持续揭示海图：四边保持主要可见范围，四个尖角按圆弧裁切，已经揭示的区域不会重新被雾覆盖。航行场景和完整海图必须复用同一份探索数据，不能出现一边已知、一边仍被覆盖的状态。

航行场景继续复用海战透明白色像素水墨海雾组件 `white_ink_mist_v1.png`。8 世界单位细格记录永久揭示；约 224 世界单位粗格只负责航行画面的雾笔美术，按海战同款确定性哈希每约四格布置一张 4.4–5.5 格宽、带稳定尺寸和位置偏移的雾纹。完整海图不采样白雾或替代海面，只读取同一探索位图的 Alpha 并输出纯黑未知区，同时追加仅用于显示的局部 Alpha 平滑，使探索边缘和直角转折略微圆润。两种表现都不能改变实际探索状态，也不能让未探索地点名称提前出现。

航行画面的未知区域不能直接以半透明雾叠在真实地图上。探索遮罩覆盖处先绘制世界坐标固定的水墨海水遮蔽底层，再在其上混合分格白雾；雾笔孔洞只显露该海水底层，不能提前显露岛屿、港口或大陆轮廓。完整海图的探索遮罩则输出纯黑，不显示这些底层细节。探索遮罩清除后，对应覆盖层移除，真实地图内容才显示。
