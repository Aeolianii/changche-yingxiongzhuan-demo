...
C 区作为第三张同规格分块放在 A 区正下方，其右侧预留 D 区，左侧没有相邻分块。C 区新增澄海灯岛、龙门海寨、白沙渔岛与玄潮古屿四个主岛。四岛整体位于地图左侧，灯岛与渔岛组成西侧双岛，海寨与古屿组成东侧双岛；两组在纵向位置上错开，并以宽阔纵向海峡分隔，组内和组间距离均不完全相同。C 区顶部保留纯海水与 A 区底部融合，最右侧保留远洋空白作为 D 区接口；进入交互继续使用“该岛屿即将开放”占位反馈。

D 区作为第四张同规格分块放在 B 区正下方、C 区右侧，完成 A/B/C/D 二乘二结构。D 区新增沧门礁堡、月环商港、雾岚群岛、伏波古岭与珊湾渔链五个可进入地点；左侧两岛与右侧两岛拉开形成纵向主航道，最下方链状渔岛保持独立。岛屿使用月牙、长脊、群岛与链状等非圆形轮廓，部分主岛带附岛和礁石；右侧另有小型岩礁障碍，只参与碰撞，不提供地点入口或海图标签。D 区顶部与左侧均保留纯海水重叠带，进入交互继续使用“该岛屿即将开放”占位反馈。

### 1.3 v4 生产落图目标

上述分块与地点说明记录当前运行时旧布局；生产落图尚未接入。v4 目标布局将右上改为 B 区群岛水战区，配置沧门礁堡、月环商港、雾岚群岛、伏波古岭与珊湾渔链五个地点；右下改为 D 区敌方核心海域，配置红湾卫所、东极秘岛与南澳商港三个地点。A 区四个地点和 C 区四个地点保持不变，总数仍为 16。

生产落图只更换四张背景、地点坐标、入口触发、静态碰撞和海图呈现，不改变自由航行、主动进入提示、自动事件、月相、存档或场景二往返逻辑。
...
- D 区承担敌方核心海域，三个主要地点形成由左上向右下推进的防御纵深，右下大型港城成为终局视觉地标。
- 16 个现有地点名称和交互职责在构图阶段全部保留，阶段一只确定视觉层级与坐标草案，不直接修改游戏运行内容。

北线为“南海军港—东湾水寨—沧门礁堡—雾岚群岛—月环商港”，再向南进入 D 区；南线为“川山渔村—澄海灯岛—龙门海寨—玄潮古屿—红湾卫所”。两线最终在 D 区经东极秘岛汇合并抵达南澳商港。四条分块接缝都必须用岛链、礁石走向或航道开口建立跨区联系，避免出现独立画布感。

阶段一拼接灰模、16 个地点层级、建议世界坐标和接缝约束见 `docs/assets/sea-overworld-stage1-layout.md`。该灰模是下一阶段背景重排依据，不作为运行时地图资源。
...
- 主要岛屿与 16 个地点数量保持不变。岛链采用“密—疏—空—密”的分组节奏，不使用等间距串珠式引导。
...
- 地点数量固定为 A4、B5、C4、D3。生产落图时地点中心相对批准草案只允许 `±80` 世界单位调整，入口触发必须对准最终画面中的可见码头或登陆口。
