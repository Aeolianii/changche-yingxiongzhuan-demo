...
| 海岸摆钩钓鱼 | 与守岭人交谈后前往码头旁鱼竿处确认进入；分别用鼠标左键、空格下钩，并测试重物、空钩、超时重试、暂时离开和 Esc | 比较鱼竿星芒、接近高亮、水墨交互按钮、摆钩角度、绳长、命中鱼获、收线速度、分数与倒计时 | 海岸鱼竿与鱼桶自然融入场景；可交互时有克制星芒，接近时使用与场景一、二一致的金色高亮和 300×74 水墨按钮；60 秒内达到 500 分完成；离开小游戏恢复码头出生点 | passed / targeted scene + UI tests 2026-08-12 |
