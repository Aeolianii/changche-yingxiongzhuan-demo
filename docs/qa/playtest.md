...
| 伏波单图地图启动 | 用 MCP 打开并运行 `fubo_guling.tscn` | 从登陆处沿主路移动 | `World/Background` 只有一个完整背景；相机有限跟随、不露图边，无板块接缝和地表贴片 | passed / 用户验收确认 2026-09-07 |
| 伏波岭南视觉 | 在登陆处、守岭院、古渠、校场和观景台各截图 | 与第一幕皇宫的角色比例、像素密度和清晰度对比 | 同属清晰精美像素 RPG；伏波采用明朗清新的红土缓丘、灰砖黛瓦、镬耳墙、岭南植物、古渠和海防校场；石材只作码头、渠岸和基脚，不出现灰色峡谷、石林、连续石墙、江南园林或厚重写实风 | passed / 用户验收确认 2026-09-07 |
| 伏波背景禁行 | 打开碰撞调试并靠近房屋、密林和大型树木 | 尝试进入屋后或树冠后方 | 玩家由大碰撞挡在安全活动区，不需要屋顶/树冠分层或环境 Y 排序 | passed / 用户验收确认 2026-09-07 |
| 伏波路线可读性 | 分别从登陆处、守岭院、古渠和校场观察下一段路线 | 不看任务箭头，仅凭场景移动 | 连续浅红褐主路与低噪声空地始终可辨；每个镜头能看到道路延伸或下一个地标；道路无连续木围栏、横切装饰和杂乱路障 | passed / 用户验收确认 2026-09-07 |
...
| 伏波终点门 | 完成古渠后尝试登台，再完成听鼓 | 穿越两道剧情障碍 | 古渠只开校场；听鼓后才开观景台并允许完成 | passed / 用户验收确认 2026-09-07 |
| 伏波大块碰撞 | 打开碰撞调试并沿道路边缘步行 | 尝试进入密林、房屋、山体与海岸 | 6–9 个大块碰撞整体封闭非游玩区和底图安全边界；道路无碎碰撞卡顿 | passed / 用户验收确认 2026-09-07 |
