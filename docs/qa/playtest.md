...
| 海上随机事件双槽 | 首次进入海图，并分别接触漂流木箱、龙井茶商和私盐商 | 核对初始事件，再完成木箱两分支、茶商两分支和私盐商三分支 | 初始恰有茶商和另一个随机事件；对话期间锁船并隐藏海图入口；结束后旧实例移除、视野外补位且总数不超过两个；当前槽位可随存档和伏波往返恢复 | passed / random-event + 3 focused branch suites 2026-08-14 |
| 私盐商船小范围巡航 | 私盐商船处于活动事件槽位 | 观察数秒后驶近触发对话 | 船只持续离开刷新点但不超过 120 世界单位；对话打开后停止移动，结束后实例移除并补充下一事件 | passed / random-event + salt branch suites 2026-08-14 |
| 迷雾与事件跨场景恢复 | 揭示远海并记录当前两个事件槽位后进入伏波古岭，再从码头返航；另恢复事件场景状态 | 核对海图位图、事件类型、位置和旧存档兼容 | 伏波往返恢复迷雾与两个活动事件类型；事件状态恢复保持类型、位置且不重复；无活动事件数组的旧快照按双槽规则补位 | passed / random-event state + enhanced Fubo round trip 2026-08-14 |
