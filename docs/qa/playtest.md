...
| 海图接取伏波支线 | 未接取时驶近伏波古岭，再直接尝试近岸入口 | 阅读士兵军情并点击“收到，我会前去巡视。” | 大范围近岛触发防倭巡岛军情；确认后正式接取支线，主线仍为“探索海域，完善海图”；未接任务不能登岛，独立运行伏波场景则自动补发 | passed / focused side-quest flow 2026-08-12 |
...
| 海图永久探索迷雾 | 从县令进入海图，向未开图方向航行并打开完整海图 | 核对已探索方向、未探索方向的视野边缘、黑色未知区和地点标签 | 已探索方向保持明亮；未开图方向只在屏幕最外缘保留约 48 像素窄黑雾，以最高 72% 黑度和九点采样形成柔和水墨渐变；移动约 2 像素即检查刷新，新开图黑雾在 0.16 秒内渐隐且不闪烁 | passed / parser + persistence + sea-scene smoke 2026-08-12 |
