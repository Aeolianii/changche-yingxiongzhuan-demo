...
| 海岸摆钩钓鱼 | 开局直接前往码头旁鱼竿处，并在首次完成后返回重复进入；分别用鼠标左键、空格下钩，并测试重物、空钩、超时重试、暂时离开和 Esc | 比较鱼竿星芒、接近高亮、水墨交互按钮、摆钩角度、绳长、命中鱼获、收线速度、分数与倒计时 | 海岸鱼竿与鱼桶自然融入场景；无需先接任务，可随时钓鱼；首次完成开放校场，重复完成不重复推进任务；选择离开后恢复进入小游戏前的位置 | passed / targeted scene + UI tests 2026-08-12 |
