...
|---|---|---|---|---|
| 海战教程干笔墨条 | 从关卡选择进入 1-1 | 查看第一步，触发包含换行正文的后续步骤，再收起/展开提示 | 移除顶部矩形目标卡片；720×164px 墨条、24px 金色标题和 20px 米白正文均水平居中；收起入口固定于右上角、与标题行齐平、右侧内缩 20px，正文换行时不重叠；动作达成后自动切换下一步 | passed / dedicated hint test + OpenGL capture 2026-08-17 |
