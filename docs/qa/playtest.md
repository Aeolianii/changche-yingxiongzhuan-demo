...
| 精细替代海面与战斗雾覆盖 | 新游戏进入海域并打开完整海图 | 观察未知区海面细纹、战斗雾覆盖、探索边缘色差和建筑遮蔽 | 雾下可见细小亮纹、暗流和水墨颗粒，无明显平铺；替代海面沿用大地图青蓝色，边缘无灰蓝拼接；战斗白雾保留浓淡飞白；未知岛屿与建筑不显示 | passed / generated asset inspection + focused runtime + 1344×896 OpenGL captures 2026-08-20 |
| 完整复用战斗雾与海图满框 | 新游戏进入海域并打开完整海图 | 对比海战与海图的单片雾尺度、落点、边缘衰减；检查内框上下边和地点坐标 | 海图逐项使用战斗 26 格雾逻辑，中小雾笔彼此独立且随实时隐藏状态变化；地图、雾、地点和玩家纵向铺满整个内框，无上下留白或错位 | passed / focused runtime + native fog-field inspection + 1344×896 OpenGL captures 2026-08-20 |
| 海域雾笔方向去重复 | 新游戏进入海上大地图，并打开完整海图 | 观察大块未知海域中雾笔的斜向纹理，再重复开关海图 | 原图、水平镜像、垂直镜像和双向镜像按坐标稳定混排；无贯穿区域的固定斜向平行线，重开海图不跳变，原有浓淡、孔洞和未知建筑遮蔽保持不变 | passed / C# build + focused sea/naval runtime + 1344×896 OpenGL capture 2026-08-20 |
