...
# 四张固定海战地图验收（2026-08-20）

- 连续进入随机遭遇战，确认只会抽到森林岛、岩山岛、港口小镇、河口岛四张 `24×18` 固定地图，地图来源名称与实际模板一致。
- 重复抽到同一模板时，主印章、伴生印章、散点装饰、出口和逐格地形必须完全一致；难度变化不得改变地图构图。
- 森林岛应读成上下夹出中央航道；岩山岛为上礁下山；港口小镇位于中下部且港池朝南；河口岛为上草洲下河口。四张图不得只是更换主素材而保留同一布局。
- 四张地图素材整体呈低饱和灰青水墨像素风，不出现棋盘格、矩形底色、倒置素材或与战场海面明显断层。
- 河口岛的河道必须由内陆连续流向开放海口；港镇中央港池必须保持深水进港通道，不得被建筑、海滩或单格障碍封死。
- 多格素材必须整张显示，素材覆盖范围内不得叠加单格树林、岩石或礁石；两枚印章之间至少保留一格外海间距。
- 双方布阵区、地图出口和主航路保持可用；森林、山地、海滩等阻挡规则与画面轮廓一致，不可出现船只穿过整座岛屿或看似海水却不可通行的大片误导区域。
- 战术主题专项：森林岛使用草地沙洲并保留斜向中路；岩山岛上方使用不可进入的破碎岩礁群；港口小镇使用不可进入的芦苇沙洲且南侧深水从外海连续进入内部停泊格；河口岛为 `8×10`，不可进入的险礁仍在岛屿上方，险礁与岛岸之间两行必须完整保持深水。
- 伴生印章专项：四张地图的伴生印章素材不得重复；点击移动范围时，草地沙洲、破碎岩礁群、芦苇沙洲和险礁的可见主体格均不得出现可移动高亮，任意通过性舰型都不能驶入。
- 岛水层级专项：以默认倍率和全图倍率检查四张地图，岛岸外侧依次可感知极淡深水阴影、浅水色和白色浪花，但不得出现黑色硬描边、矩形重影或遮住港口入口。
