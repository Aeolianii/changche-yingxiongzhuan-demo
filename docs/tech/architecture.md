...
海上随机事件继续由 `SeaOverworld` 创建触发区，但实际选项界面统一使用 `FieldEventDialogue`。控制器维护最多两个活动事件槽位：新海图实例固定创建龙井茶商，再从漂流木箱与私盐商中随机创建一个；任一事件完整结算并移除后，从未激活类型中选择一个位于当前相机视野外的固定清水刷新点补位。同类型在后续轮换中可以再次出现，但同一时刻只允许一个实例。对话打开时锁定船只控制、暂停私盐商船巡航并隐藏海图入口；当前两个事件的类型与位置写入正式场景快照和伏波返航上下文，读取时先恢复快照再补足槽位。旧版三项一次性完成布尔值只用于兼容缺少活动事件数组的旧快照。
...
- 海上大地图场景快照记录玩家位置与朝向、探索引导阶段、月相日数、当前两个随机事件的类型与位置，以及倭寇营地状态；海图迷雾位集合属于全局世界状态，确保从场景二或伏波古岭往返后仍能恢复已揭示海域。
