...
- 伏波校场道路上的栏杆是永久可见的环境装饰，保留与素材轮廓对应的局部实体碰撞，玩家可从两侧绕行；栏杆不承担任务门控，校场小游戏是否可进入只由任务阶段决定。伏波主任务栏与任务界面同时读取阶段和独立的 `keeper_intro_completed`：登岛不会自动完成“寻找守岭人”，首次对话结束后才勾选该步骤；首次钓鱼前主目标指向码头旁鱼竿，成功后同步切换为前往古校场。结算笔触只报告“渔获满舱，收竿归岸”，不承担任务导航。
