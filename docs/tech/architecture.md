...
海域探索迷雾由独立 `SeaFogOfWar` 组件在运行时创建。组件用低分辨率位图记录永久揭示单元，以当前 `Camera2D` 可见世界矩形在玩家航行路径上增量盖章；揭示矩形相对视野四边内缩 48 世界像素，使尚未开图的方向只在屏幕最外缘保留一条窄白雾，而已经探索过的方向保持透明。初始化时还读取 `WorldCollision/NorthwestCoast` 的碰撞多边形，将左上大陆轮廓标记为默认已知。生成的 Alpha 纹理只表达未知区域遮罩，同时供世界遮罩与 `SeaMapScreen` 使用；最终颜色和纹理由两处材质采样 `SeaFogOfWar` 使用海战 `assets/naval/ui/fog/white_ink_mist_v1.png` 预合成的共享雾笔纹理。世界遮罩位于地点/事件之上、玩家船只之下，完整海图按同一位图隐藏未探索地图与地点名称。
