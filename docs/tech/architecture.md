...
世界遮罩使用 `sea_world_fog_edge.gdshader` 在局部网格中查找最近的相反迷雾单元，计算单一有符号边界距离，再通过一次 `smoothstep` 生成连续 Alpha。迷雾表现由 `SeaFogOfWar` 生成两张原生画布纹理：航行画面使用 26 世界单位表现格和两倍探索遮罩分辨率；完整海图使用 870×510 画布和 26 海图像素表现格。每个格中心都映射到同一份 8 世界单位探索位图判断隐藏状态，九宫格邻居也只统计仍隐藏且在界内的表现格。随后两处逐项执行海战 `Hash01(x,y,53)`、`seed % 4`、4.4–5.5 格宽、0.6875 高宽比、偏移、0.38 Alpha 与 0.76 边缘衰减。每次探索遮罩实际变化后重建并更新两个既有 `ImageTexture` 实例，复现战斗中雾笔中心揭示后整张移除的行为；海图不再缩小世界雾纹理，避免数千张小雾笔被平均成一层。永久开图仍使用 8 像素逻辑网格，旧版 16 像素存档读取规则不变。
