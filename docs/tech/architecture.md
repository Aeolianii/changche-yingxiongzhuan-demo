...
- 海图主线固定为“探索海域，完善海图”，伏波古岭以支线形式存在。海图在伏波古岭外围建立半径 760 的一次性军情触发区，确认士兵报告后写入 `GameState.world_state.fubo_side_quest`；未接支线时近岸入口只打开同一报告。岛内独立调试会自动补发该支线。任务界面通过 `side_quest_selected` 逐层通知全局 HUD 与当前场景，点击任一未完成支线都会更新左侧追踪栏，并把 `tracked_side_quest` 写入世界状态随正式存档保留。伏波阶段推进至 4 时写入 `completed` 并自动清除追踪：任务从“进行中”移入“已完成”，左侧支线卡片隐藏；旧存档缺少该字段时按阶段 4 自动迁移为完成态。
