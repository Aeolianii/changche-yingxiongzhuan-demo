...
世界遮罩使用 `sea_world_fog_edge.gdshader` 对窄边迷雾做九点邻域采样、固定在世界纹理坐标上的双尺度水墨扭曲和最高 0.72 黑色透明度混合，使视野内侧原本平直的矩形黑雾形成轻微起伏、细节不完全一致的自然轮廓，同时不随时间采样，避免航行时边缘抖动。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑开图状态立即写入位集合，视觉 Alpha 则在 0.16 秒内逐帧渐隐，消除整列迷雾瞬间清空造成的闪烁；开场、读档和默认已知大陆使用立即揭示，避免载入时闪黑。永久开图仍使用 16 像素逻辑网格，存档格式不变。`SeaMapScreen` 仍在共享迷雾纹理上使用半径更大的海图专用材质，圆润完整海图中的探索轮廓。地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种柔边都不参与探索判定和存档。
