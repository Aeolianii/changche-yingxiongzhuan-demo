...
海上随机事件继续由 `SeaOverworld` 创建触发区，但实际双选项界面统一使用 `FieldEventDialogue`。对话打开时锁定船只控制并隐藏海图入口；漂流木箱完成后写入场景快照并永久移除，龙井茶商和私盐商保留驶离后再次接触的原型规则。事件结算不写入正式背包，只保存海图原型所需的银钱与一次性完成状态。
...
- 海上大地图场景快照记录玩家位置与朝向、探索引导阶段、月相日数、事件原型状态和临时银钱；海图迷雾位集合属于全局世界状态，确保从场景二或伏波古岭往返后仍能恢复已揭示海域。
