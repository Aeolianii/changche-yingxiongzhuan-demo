...
海上随机事件继续由 `SeaOverworld` 创建触发区，但实际选项界面统一使用 `FieldEventDialogue`。控制器维护最多两个活动事件槽位，每次创建海图实例都重新抽取事件与清水刷新点，不恢复上次的活动类型、位置或巡航相位。龙井茶商完成状态由 `GameState.world_state` 单独持久化：未完成时固定占一个槽，完成后不再强制出现；其余槽位从漂流木箱、私盐商和“雾中可疑身影”中抽取。海怪事件只占一种类型，每次实例化随机绑定三种透明雾影素材和对应立绘之一；其 `ShaderMaterial` 以亮度遮罩将时间噪声主要作用在白雾上，实现低速漂移和明暗呼吸，同时让暗色剪影保持稳定。任一事件结算后，从当前未激活类型中选择一个位于相机视野外的清水点补位。私盐商复用海盗巡游/停驻时间区间和出生半径目标选择，但没有警戒、追逐和追敌返航；对话打开时统一暂停动态实体。
