...
海域探索迷雾由独立 `SeaFogOfWar` 组件在运行时创建。组件使用低分辨率位图记录永久揭示单元，以当前 `Camera2D` 可见世界矩形在玩家航行路径上增量盖章，并把左上大陆轮廓标记为默认已知。世界遮罩位于地点和事件之上、玩家船只之下；`SeaMapScreen` 复用同一迷雾纹理，并可使用只影响显示的水墨柔边材质。地点标签仍查询原始探索位集合，因此柔边不参与探索判定。
...
全局海图状态由 `GameState` 常驻内存管理，存档时把位集合编码为带尺寸、单元大小和版本的 Base64 数据。读取旧存档时该字段可缺省，海上大地图会回退到默认大陆与出生视野；开始新游戏只清空运行时世界状态，不删除正式存档文件。
