...
海战地图逻辑继续由 `LevelMapSpec`/`BattleMap` 的逐格 `TerrainType` 表达；多格地形印章只是在同一地图上附加的表现元数据，记录纹理路径、左上格、占地尺寸和旋转象限。随机地图生成器先按印章掩码一次写入森林、草地、海滩与陆河等逻辑格，再执行既有布阵区避让、出口保护和 BFS 连通性校验；生成失败时重试或回退，不允许把河流降级成无源无口的散点。部署控制器只负责把印章元数据随地图规格复制到战斗地图，`NavalGridView` 在海面和格面之上整张绘制印章，并跳过覆盖区域的单格地形精灵。图片 Alpha 不参与规则判定，战争迷雾、范围提示和舰船层仍绘制在印章之上。
