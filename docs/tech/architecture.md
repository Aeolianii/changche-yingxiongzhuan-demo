...
海怪讨伐第一、二场在各自 `MapScheme` 上附加覆盖地貌的透明大图印章。`NavalGridView` 在印章范围内以海水绘制底层格面、绘制完整图像并跳过单格障碍贴图；原 `TerrainType` 继续决定通行、碰撞和战术效果。图片的透明区保留动态海水与战术格，迷雾和交互高亮仍绘制在上层。
