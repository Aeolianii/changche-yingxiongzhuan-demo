...
世界遮罩使用 `sea_world_fog_edge.gdshader` 在局部网格中查找最近的相反迷雾单元，计算单一有符号边界距离，再通过一次 `smoothstep` 生成连续 Alpha；它不再累加多个遮罩偏移采样层，因此不会产生半透明等高线。迷雾表现另由 `SeaFogOfWar` 在初始化时建立约 84、56、40 世界单位三套表现格，以不同哈希盐把 `white_ink_mist_v1.png` 预合成为背景、中景、细节三张世界坐标纹理。每层继承海战稀疏落点和尺寸变化，同时使用独立透明度与水平/垂直镜像；世界和完整海图 Shader 分别以灰蓝、蓝白、灰白混合三层，不再由一张约千世界单位宽的大云纹覆盖画面。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑状态与视觉遮罩同步清除，不保留逐格渐隐 Alpha。永久开图使用 8 像素逻辑网格，旧版 16 像素存档会在读取时无损扩展到新网格。`SeaMapScreen` 仍在共享探索纹理上使用海图专用材质圆润轮廓；地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此表现格、柔边和雾笔纹理都不参与探索判定和存档。
