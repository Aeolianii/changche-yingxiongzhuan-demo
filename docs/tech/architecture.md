...
世界遮罩使用 `sea_world_fog_edge.gdshader` 对窄边迷雾做 5×5 高斯采样、固定在世界纹理坐标上的双尺度水墨扭曲、过渡区低幅 Alpha 抖动和最高 0.72 黑色透明度混合，使视野内侧原本平直的矩形黑雾形成连续且无明显色阶的自然轮廓，同时不随时间采样，避免航行时边缘抖动。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑状态与视觉遮罩同步清除，不再保留逐格渐隐 Alpha，避免持续航行时相邻批次形成矩形明暗台阶；柔化过程完全交给扩大后的世界坐标高斯与水墨扰动完成。永久开图使用 8 像素逻辑网格，旧版 16 像素存档会在读取时无损扩展到新网格。`SeaMapScreen` 仍在共享迷雾纹理上使用半径更大的海图专用材质，圆润完整海图中的探索轮廓。地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种柔边都不参与探索判定和存档。
