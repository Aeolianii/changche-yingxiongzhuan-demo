...
海战地图逻辑继续由 `LevelMapSpec`/`BattleMap` 的逐格 `TerrainType` 表达；多格地形印章只是在同一地图上附加的表现元数据，记录纹理路径、左上格、占地尺寸和旋转象限。`RandomMapGenerator` 现维护森林岛、岩山岛、港口小镇、河口岛四套固定 `24×18` 战术模板，种子只选择模板，不再随机寻找印章落点或生成山地簇、浅滩和礁石散点。模板分别编码双岛斜向夹道、偏置主山与 `7×4` 破碎岩礁群、连续深水港池与 `6×4` 芦苇沙洲、`8×10` 河口岛与其上方险礁之间的两行深水通道；森林图继续使用草地沙洲。破碎岩礁群、芦苇沙洲与重做险礁的整个占格矩形全部编码为陆地，任何舰型均不可进入；草地沙洲继续按透明轮廓阻挡，单格 `Reef` 的原有通过性规则不变。固定印章允许相邻但禁止矩形重叠，并继续通过 BFS 连通性校验。非标准尺寸无法容纳模板时才回退开阔海域。`RandomEncounterGenerator` 不再混用旧方案图和伪随机变体，所有随机遭遇统一从这四张模板抽取。部署控制器把印章元数据随地图规格复制到战斗地图，`NavalGridView` 在海面和格面之上先以同一 Alpha 轮廓绘制深水阴影、浅水色、白色浪花三层轻微扩张，再整张绘制正式印章，并跳过覆盖区域的单格地形精灵。图片 Alpha 不参与规则判定，战争迷雾、范围提示和舰船层仍绘制在印章之上。
