...
海域探索迷雾由独立 `SeaFogOfWar` 组件在运行时创建。组件用低分辨率位图记录永久揭示单元，以当前 `Camera2D` 可见世界矩形在玩家航行路径上增量盖章；揭示矩形相对视野四边内缩 48 世界像素，使尚未开图的方向只在屏幕最外缘保留一条窄白雾，而已经探索过的方向保持透明。初始化时还读取 `WorldCollision/NorthwestCoast` 的碰撞多边形，将左上大陆轮廓标记为默认已知。生成的 Alpha 纹理只表达未知区域遮罩，同时供世界遮罩与 `SeaMapScreen` 使用；最终颜色和纹理由两处材质共同采样海战 `assets/naval/ui/fog/white_ink_mist_v1.png`。世界遮罩位于地点/事件之上、玩家船只之下，完整海图按同一位图隐藏未探索地图与地点名称。

世界遮罩使用 `sea_world_fog_edge.gdshader` 在局部网格中查找最近的相反迷雾单元，计算单一有符号边界距离，再通过一次 `smoothstep` 生成连续 Alpha；它不再累加多个遮罩偏移采样层，因此不会产生半透明等高线。边缘只叠加固定在世界纹理坐标上的轻微水墨位移与低幅抖动；雾面以错位、镜像的白雾素材采样合成连续覆盖，并保留低透明度底雾消除素材透明边缘造成的平铺孔洞。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑状态与视觉遮罩同步清除，不保留逐格渐隐 Alpha。永久开图使用 8 像素逻辑网格，旧版 16 像素存档会在读取时无损扩展到新网格。`SeaMapScreen` 仍在共享迷雾纹理上使用海图专用材质，圆润完整海图中的探索轮廓并采样同一白雾素材。地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种柔边和白雾纹理都不参与探索判定和存档。

海图迷雾材质使用基于 UV 的确定性多尺度值噪声扰动邻域采样中心与 Alpha 过渡阈值，并用独立缩放的错位白雾素材层打散重复。噪声不依赖时间，且只在原始迷雾的过渡带内改变轮廓，因此不会造成闪烁、内部雾孔或探索状态漂移。
