...
世界遮罩使用 `sea_world_fog_edge.gdshader` 在局部网格中查找最近的相反迷雾单元，计算单一有符号边界距离，再通过一次 `smoothstep` 生成连续 Alpha。迷雾表现由 `SeaFogOfWar` 生成两张原生画布纹理：航行画面使用 26 世界单位表现格和两倍探索遮罩分辨率；完整海图使用 870×510 画布和 26 海图像素表现格。每个格中心都映射到同一份 8 世界单位探索位图判断隐藏状态，九宫格邻居也只统计仍隐藏且在界内的表现格。战斗与海域先对 `Hash01` 做两轮确定性整数雪崩混合；每个 2×2 表现格再由独立盐值哈希选择一个候选落点，稳定保持四分之一密度，且不再形成原 `seed % 4` 的斜向周期或纯随机大空洞。候选格继续使用 4.4–5.5 格宽、0.6875 高宽比、偏移、0.38 Alpha 与 0.76 边缘衰减。海域每个尺寸还缓存原图、水平镜像、垂直镜像和双向镜像四种雾笔，使用同一 `seed` 的高位稳定选择方向，进一步打散素材固定斜向笔触。每次探索遮罩实际变化后重建并更新两个既有 `ImageTexture` 实例，复现战斗中雾笔中心揭示后整张移除的行为；海图不再缩小世界雾纹理，避免数千张小雾笔被平均成一层。永久开图仍使用 8 像素逻辑网格，旧版 16 像素存档读取规则不变。

`SeaMapScreen` 的 `_map_content_rect` 固定为整个 870×510 内框。地图分块用 `Vector2(870,510) / world_size` 分别计算 X/Y 缩放，雾层、地点与玩家标记复用同一非等比映射；这样纵向轻微拉伸但不会发生层间错位。
...
两个探索迷雾 Shader 采样 `sea_concealment_ink_pixel_v1.png` 作为未知区域的世界坐标海水遮蔽细节。该纹理按覆盖世界的 `UV` 单次采样，不通过 `fract` 高频重复；Shader 只提取 1536×1024 素材中的细亮纹与暗流亮度，用其调制与大地图一致的青蓝基准色。探索遮罩存在时，材质输出为不透明海水底层与战斗雾笔颜色的混合，而不是让雾笔 Alpha 直接暴露底下的地图；探索遮罩清除时整个材质输出透明。这样无需拆分真实地图资源，也能保证探索边缘色相连续、雾中孔洞只看到海水、未知地形直到揭示后才出现。
