...
场景一完成后由本场景脚本启动一次性计时器，并调用独立的 `ChapterTransition` Canvas UI 播放 1.5 秒南下行程。过场完成时在场景树根节点写入一次性入口标记，再调用 `SceneTree.change_scene_to_file()` 切换到 `res://scenes/Scene2.tscn`。切换失败时移除标记、关闭过场并恢复完成态 UI，允许玩家重试。场景一和场景二的人物交互统一使用“邻近角色高亮 + 屏幕底部水墨按钮”，键盘交互键与鼠标点击按钮会进入同一对话流程。
