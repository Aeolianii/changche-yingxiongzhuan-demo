...
世界遮罩使用 `sea_world_fog_edge.gdshader` 对窄边迷雾做轻量邻域采样、稳定水墨扰动和 0.9 透明度混合，使黑雾与已照亮海域平滑衔接且不会大面积压暗当前视野。`SeaMapScreen` 仍在共享迷雾纹理上使用半径更大的海图专用材质，圆润完整海图中的探索轮廓。地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种柔边都不参与探索判定和存档。
