...
海域探索迷雾由独立 `SeaFogOfWar` 组件在运行时创建。组件用低分辨率位图记录永久揭示单元，以当前 `Camera2D` 可见世界圆角矩形在玩家航行路径上增量盖章；揭示范围相对视野四边内缩 48 世界像素，圆角半径按内缩后视野短半轴的固定比例计算，使尚未开图的方向只在屏幕最外缘保留一条窄白雾，同时避免四角形成直角矩形。初始化时还读取 `WorldCollision/NorthwestCoast` 的碰撞多边形，将左上大陆轮廓标记为默认已知。生成的 Alpha 纹理只表达未知区域遮罩，同时供世界遮罩与 `SeaMapScreen` 使用。世界材质继续组合海战 `assets/naval/ui/fog/white_ink_mist_v1.png` 与替代海面；海图专用材质只读取共享 Alpha 并输出纯黑色。世界遮罩位于地点/事件之上、玩家船只之下，完整海图按同一位图隐藏未探索地图与地点名称。

世界遮罩使用 `sea_world_fog_edge.gdshader` 在局部网格中查找最近的相反迷雾单元，计算单一有符号边界距离，再通过一次 `smoothstep` 生成连续 Alpha；它不再累加多个遮罩偏移采样层，因此不会产生半透明等高线。迷雾表现另由 `SeaFogOfWar` 在初始化时建立约 224 世界单位的粗格，并用海战 `Hash01(x, y, 53)` 等价哈希、每约四格一张、4.4–5.5 格宽和稳定位置偏移，把 `white_ink_mist_v1.png` 预合成为一张世界坐标固定的密度纹理；该纹理只供航行世界材质使用。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑状态与视觉遮罩同步清除，不保留逐格渐隐 Alpha。永久开图使用 8 像素逻辑网格，旧版 16 像素存档会在读取时无损扩展到新网格。`SeaMapScreen` 在共享探索纹理上使用 `sea_map_fog_soft_edge.gdshader` 输出纯黑并圆润轮廓；地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种颜色表现和柔边都不参与探索判定或存档。

海图迷雾材质使用基于 UV 的确定性多尺度值噪声扰动探索遮罩的邻域采样中心与 Alpha 过渡阈值，最终颜色固定为纯黑，不再采样分格白雾或替代海面。噪声不依赖时间且只改变探索边缘，因此不会造成闪烁或探索状态漂移。
