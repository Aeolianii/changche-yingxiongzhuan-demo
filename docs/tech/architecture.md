...
地图分块共享同一世界坐标系：A 基础图位于左上，B 东部扩展位于右上并沿西缘短距离重叠，C 南部扩展位于 A 正下方并沿北缘短距离重叠；D 预留在 C 右侧但尚未接入。`MAP_SIZE`、玩家 `movement_bounds` 和 `Camera2D` 限制使用 A/B/C 组成的 L 形分块包络，空缺 D 区入口由碰撞边界阻挡。B 使用横向 Alpha 融合，C 使用纵向 Alpha 融合；`World/EastBackground` 与 `World/CBackground` 均序列化进场景，运行时脚本只校正参数。完整海图按每个分块的世界矩形投影到统一预览面板，地点和玩家标记继续以总世界尺寸归一化定位。
