...
世界遮罩使用 `sea_world_fog_edge.gdshader` 在局部网格中查找最近的相反迷雾单元，计算单一有符号边界距离，再通过一次 `smoothstep` 生成连续 Alpha；它不再累加多个偏移采样层，因此不会产生半透明等高线。边缘仅叠加固定在世界纹理坐标上的轻微水墨位移与低幅抖动，并以最高 0.72 黑色透明度混合。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑状态与视觉遮罩同步清除，不保留逐格渐隐 Alpha。永久开图使用 8 像素逻辑网格，旧版 16 像素存档会在读取时无损扩展到新网格。`SeaMapScreen` 仍在共享迷雾纹理上使用海图专用材质，圆润完整海图中的探索轮廓。地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种柔边都不参与探索判定和存档。
