...
海盗船是独立的 `CharacterBody2D` 动态海图实体，由 `SeaOverworld` 在场景初始化后通过物理点查询选择可航出生点并实例化。海盗脚本管理出生半径内的巡游/停驻、带迟滞的追逐判定、出生点追击活动边界和独立返航状态；返航期间不进行警戒判定，抵达出生点后才重启巡逻。脚本同时管理四向船体与复用尾流表现；接触玩家后向海图场景发出战斗请求，不直接打开 UI 或切换战斗场景。海图控制器统一暂停玩家、其他海盗和瞬态海图交互，并通过 `FieldEventDialogue` 显示当前“海战界面即将开放”占位提示。海盗运行时状态不进入正式存档与返航上下文。
