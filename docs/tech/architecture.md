...
海战地图逻辑继续由 `LevelMapSpec`/`BattleMap` 的逐格 `TerrainType` 表达；多格地形印章只是在同一地图上附加的表现元数据，记录纹理路径、左上格、占地尺寸和旋转象限。随机地图生成器维护数据驱动的印章池，先按种子抽取一枚主题主印章与一枚小型伴生印章，再对每个候选矩形执行边界、布阵区、出口、既有印章占用检查，并按掩码一次写入森林、草地、海滩、陆河、山地、浅滩、礁石、港口与小镇等逻辑格；随后继续执行既有散点生成和 BFS 连通性校验。生成失败时重试或回退，不把完整印章降级成无上下文散点。部署控制器只负责把印章元数据随地图规格复制到战斗地图，`NavalGridView` 在海面和格面之上整张绘制印章，并跳过覆盖区域的单格地形精灵。图片 Alpha 不参与规则判定，战争迷雾、范围提示和舰船层仍绘制在印章之上。
