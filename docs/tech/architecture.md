...
海域探索迷雾由独立 `SeaFogOfWar` 组件在运行时创建。组件用低分辨率位图记录永久揭示单元，以当前 `Camera2D` 可见世界矩形在玩家航行路径上增量盖章；揭示矩形相对视野四边内缩 48 世界像素，使尚未开图的方向只在屏幕最外缘保留一条窄黑雾，而已经探索过的方向保持透明。初始化时还读取 `WorldCollision/NorthwestCoast` 的碰撞多边形，将左上大陆轮廓标记为默认已知。生成的黑色 Alpha 纹理同时供世界遮罩与 `SeaMapScreen` 使用。世界遮罩位于地点/事件之上、玩家船只之下，完整海图按同一位图隐藏未探索地图与地点名称。

世界遮罩使用 `sea_world_fog_edge.gdshader` 对窄边迷雾做 5×5 高斯采样、固定在世界纹理坐标上的双尺度水墨扭曲、过渡区低幅 Alpha 抖动和最高 0.72 黑色透明度混合，使视野内侧原本平直的矩形黑雾形成连续且无明显色阶的自然轮廓，同时不随时间采样，避免航行时边缘抖动。视野揭示的移动检测距离为 2 世界像素，比迷雾逻辑单元更频繁地检查相机与目标位置，航行时能更及时刷新新进入的单元。新单元的逻辑开图状态立即写入位集合，视觉 Alpha 则在 0.4 秒内逐帧渐隐，消除整列迷雾瞬间清空造成的闪烁；开场、读档和默认已知大陆使用立即揭示，避免载入时闪黑。永久开图提高为 8 像素逻辑网格，旧版 16 像素存档会在读取时无损扩展到新网格。`SeaMapScreen` 仍在共享迷雾纹理上使用半径更大的海图专用材质，圆润完整海图中的探索轮廓。地点标签直接查询 `SeaFogOfWar` 的原始位集合，因此两种柔边都不参与探索判定和存档。
