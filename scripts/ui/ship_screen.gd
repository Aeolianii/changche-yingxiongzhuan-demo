...
# CHG-20260818：小地图阵型编辑器——玩家布阵区（与 C# NavalDeploymentController.PlayerZone 同源：x[1,23) y[2,34)）。
# 小地图按 1:1 记录海战布阵坐标，保存到预设 Formation，布阵侧直接按坐标摆位。
const FORMATION_ZONE_X := 1
const FORMATION_ZONE_Y := 2
const FORMATION_ZONE_W := 22
const FORMATION_ZONE_H := 32
...
func _build_fleet_config_page() -> void:
	...
	var hint := _make_label("点「摆位」→ 点小地图格子放置；点小地图舰块选中，再点空格移动；「旋转朝向」调朝向；「一字排开」重置为默认阵型（全舰横排）。", 12, TEXT_MUTED)
	...
	_minimap.cell_size = 12
	_minimap.grid_origin = Vector2(4, 4)
	_minimap.position = Vector2(268, 94)
	_minimap.size = Vector2(FORMATION_ZONE_W * 12 + 8, FORMATION_ZONE_H * 12 + 8)
	...
	var rotate_button := _make_action_button("旋转朝向", Vector2(268, 494), Vector2(88, 28))
	...
	var deselect_button := _make_action_button("退选", Vector2(362, 494), Vector2(58, 28))
	...
	var row_button := _make_action_button("一字排开", Vector2(426, 494), Vector2(88, 28))
	...
	separator.position = Vector2(18, 534)
...
func _build_fleet_preset_bar() -> void:
	...
	_preset_status.position = Vector2(18, 544)
	...
	caption.position = Vector2(18, 568)
	...
	_preset_name.position = Vector2(58, 568)
	...
	var save_button := _make_action_button("保存", Vector2(202, 568), Vector2(56, 28))
	save_button.name = "PresetSave"
	save_button.pressed.connect(_save_fleet_preset)
	_preset_panel.add_child(save_button)

	var default_button := _make_action_button("设默认", Vector2(262, 568), Vector2(88, 28))
	...
	_active_label.position = Vector2(356, 568)
	...
	scroll.position = Vector2(18, 604)
...
func _sync_minimap() -> void:
	...
	for ship_id in _placements:
		...
		block["len"] = _naval_length(str(_type_by_id.get(ship_id, "")))
...
func _on_minimap_input(event: InputEvent) -> void:
	...
	if cell.x < FORMATION_ZONE_X or cell.y < FORMATION_ZONE_Y:
		return
	for ship_id in _placements:
		if not _checked.get(ship_id, false):
			continue
...
func apply_default_formation() -> void:
	# 默认阵型 = 全舰一字排开：按 economy 顺序排成单行（朝东、同行、船头递进不重叠），行满换行。
	var zone := _formation_zone()
	var y := zone.position.y + 3
	var xs := zone.position.x + 1
	for ship in _ships:
		var ship_id := str(ship.get("id", ""))
		if not _checked.get(ship_id, false):
			continue
		var len := _naval_length(str(ship.get("type_id", "")))
		if xs + len - 1 >= zone.end.x:
			y += 2
			xs = zone.position.x + 1
		_placements[ship_id] = {"x": xs, "y": y, "facing": "east"}
		xs += len + 1
	_active_block = ""
	_sync_minimap()
	if _preset_status != null:
		_preset_status.text = "已按默认阵型一字排开（朝东）"
...
class FormationGrid:
	...
	var blocks: Dictionary = {}  # ship_id -> {x, y, facing, len}
	var active_id := ""

	func screen_to_cell(local_pos: Vector2) -> Vector2i:
		# 局部坐标 → 格子坐标（超出网格返回 (-1,-1)）。
	...
	func _draw() -> void:
		...
		for y in range(zone_h):
			for x in range(zone_w):
				draw_rect(_cell_rect(x, y), Color(0.16, 0.24, 0.20, 0.35), false, 1.0)
		var zone_rect := Rect2(
			grid_origin.x + zone_x * cell_size,
			grid_origin.y + zone_y * cell_size,
			(zone_w - zone_x) * cell_size,
			(zone_h - zone_y) * cell_size)
		draw_rect(zone_rect, Color(0.28, 0.58, 0.52, 0.10), true)
		draw_rect(zone_rect, Color(0.36, 0.68, 0.58, 0.70), false, 1.0)
		for ship_id in blocks:
			...
			for cell in cells:
				var rect := _cell_rect(cell.x, cell.y).grow(-1.0)
				draw_rect(rect, Color(0.96, 0.78, 0.28, 0.95) if ship_id == active_id else Color(0.16, 0.42, 0.34, 0.9), true)
				draw_rect(rect, Color(0.02, 0.04, 0.035, 1.0), false, 1.0)
