...
func _run() -> void:
	...
	_expect(game_state.call("upgrade_economy_ship", "ship_001", "speed").get("ok", false), "GameState must expose persistent ship upgrades.")
	...
	_expect(restored["pay"] == 475 and restored["items"]["grouper"] == 2 and restored["ship_upgrade_materials"]["canvas"] == 14, "Economy and isolated ship-upgrade materials must survive disk round trip.")
	...
	_expect(restored["ships"][0]["upgrades"]["speed"] == 1, "Ship upgrade levels must survive disk round trip.")
