...
func _run() -> void:
	...
	_expect((screen.find_child("FleetLabel", true, false) as Label).text.contains("3 艘"), "Inventory must display the three-ship starting fleet count without a capacity limit.")
