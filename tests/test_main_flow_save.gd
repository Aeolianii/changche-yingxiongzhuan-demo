...
func _verify_palace_restore() -> void:
	...
	_expect((scene.get_node("UI/ExplorationHUD/QuestTracker/MainQuest/TaskName") as Label).text == "听取内侍传召", "Palace must restore the task projection for WAIT_TALK.")
...
func _verify_scene_two_restore() -> void:
	...
	_expect("中军军官" in (scene.get_node("UI/ExplorationHUD/QuestTracker/MainQuest/Objective") as Label).text, "Scene2 must restore the officer-report objective.")
	(scene.find_child("SaveGameButton", true, false) as Button).pressed.emit()
	await process_frame
