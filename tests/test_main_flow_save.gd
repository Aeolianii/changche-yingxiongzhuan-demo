...
func _verify_sea_restore() -> void:
	...
	_expect((root.get_node("ExplorationUI/HUD/QuestTracker/MainQuest/TaskName") as Label).text == "探索海域，完善海图", "Sea overworld must restore main's chart-completion task title.")
	(root.get_node("ExplorationUI/HUD").find_child("SaveGameButton", true, false) as Button).pressed.emit()
	await process_frame
