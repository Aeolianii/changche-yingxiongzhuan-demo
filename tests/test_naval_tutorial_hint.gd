...
func _run() -> void:
	...
	_expect(is_equal_approx(float(hud.call("SurrenderPanelWidth")), 720.0) and is_equal_approx(float(hud.call("SurrenderPanelHeight")), 164.0), "Surrender prompt must match the tutorial 720 by 164 brush layout.")
	_expect(hud.call("SurrenderTitleText") == "劝降交涉" and int(hud.call("SurrenderTitleFontSize")) == 24, "Surrender prompt title must match the tutorial 24 px gold hierarchy.")
	_expect(int(hud.call("SurrenderCaptionFontSize")) == 20 and int(hud.call("SurrenderButtonFontSize")) == 20, "Surrender body and action text must match the tutorial 20 px copy.")
	_expect(int(hud.call("SurrenderTitleOutlineSize")) == 4 and int(hud.call("SurrenderCaptionOutlineSize")) == 4, "Surrender title and body must match the tutorial strong black outlines.")
	_expect(is_equal_approx(float(hud.call("SurrenderOfferButtonWidth")), 80.0) and is_equal_approx(float(hud.call("SurrenderOfferButtonHeight")), 44.0), "Surrender action must match the tutorial 80 by 44 toggle hit area.")
