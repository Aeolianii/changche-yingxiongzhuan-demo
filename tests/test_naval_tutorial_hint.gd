...
func _run() -> void:
	...
	_expect(is_equal_approx(float(hud.call("SurrenderPanelWidth")), 520.0) and is_equal_approx(float(hud.call("SurrenderPanelHeight")), 200.0), "Surrender dispatch must use the roomy 520 by 200 maritime layout.")
	...
	_expect(is_equal_approx(float(hud.call("SurrenderOfferButtonWidth")), 88.0) and is_equal_approx(float(hud.call("SurrenderOfferButtonHeight")), 34.0), "Surrender action must use the compact 88 by 34 button size.")
