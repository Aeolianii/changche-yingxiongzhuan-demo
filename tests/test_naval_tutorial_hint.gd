...
func _run() -> void:
	...
	_expect(is_equal_approx(float(level_play.call("HintBarWidth")), 600.0), "Tutorial brush strip must expand to 600 px wide.")
	_expect(is_equal_approx(float(level_play.call("HintBarHeight")), 136.0), "Tutorial brush strip must expand to 136 px high.")
