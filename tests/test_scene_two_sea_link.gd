...
func _run() -> void:
	...
	_expect(task_name.text == "探索海域，完善海图", "Post-drill Scene2 task was not prepared for chart exploration.")
	...
	_expect(sea_player.global_position.is_equal_approx(Vector2(760, 1130)), "Sea-overworld entry must spawn on collision-free water inside South Sea Harbor's crescent basin.")
	await physics_frame
	await physics_frame
	...
	_expect(returned_task.text == "探索海域，完善海图", "Returning to Scene2 must preserve the chart-exploration task state.")
	...
	_expect(quest_screen.visible and "探索海域，完善海图" in selected_title.text, "Returned Scene2 quest screen must retain the chart-exploration task.")
